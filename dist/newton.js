...
const diffX1 = (x1, x2, x3) => {
    return (8 * x1 - 3 * x3);
};
const diffX2 = (x1, x2, x3) => {
    return (6 * x2 - x3);
};
const diffX3 = (x1, x2, x3) => {
    return (2 * x3 - 3 * x1 - x2 + 8);
};
const grad = (x1, x2, x3) => {
    return Math.sqrt(Math.pow(diffX1(x1, x2, x3), 2) + Math.pow(diffX2(x1, x2, x3), 2) + Math.pow(diffX3(x1, x2, x3), 2));
};
let x = 1;
let y = 1;
let z = 1;
...
do {
    ...
    if (grad(x1, y1, z1) < e)
        flag = false;
    console.log(++iterations, "|", x1.toFixed(4), y1.toFixed(4), z1.toFixed(4), "|", fun(x1, y1, z1), "|", grad(x1, y1, z1).toFixed(4));
    let b1 = Math.max((Math.pow(grad(x1, y1, z1), 2)) / (Math.pow(grad(x, y, z), 2)), 0);
